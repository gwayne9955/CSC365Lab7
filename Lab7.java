...
public class Lab7 {
    ...
    private static void r5() throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(System.getenv("L7_JDBC_URL"), System.getenv("L7_JDBC_USER"),
            ...
            String sql = "select roomName, round(sum((case      when (checkout like '2019-01-%')      then datediff(checkout,checkin)*rate      else 0      end  ))) as m1 ,  round(sum((case      when (checkout like '2019-02-%')      then datediff(checkout,checkin)*rate      else 0      end  )) ) as m2,  round(sum((case      when (checkout like '2019-03-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m3,  round(sum((case      when (checkout like '2019-04-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m4,  round(sum((case      when (checkout like '2019-05-%')      then datediff(checkout,checkin)*rate      else 0      end  )) ) as m5,  round(sum((case      when (checkout like '2019-06-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m6,  round(sum((case      when (checkout like '2019-07-%')      then datediff(checkout,checkin)*rate      else 0      end  )) ) as m7,  round(sum((case      when (checkout like '2019-08-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m8,  round(sum((case      when (checkout like '2019-09-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m9,  round(sum((case      when (checkout like '2019-10-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m10,  round(sum((case      when (checkout like '2019-11-%')      then datediff(checkout,checkin)*rate      else 0      end  )) ) as m11,  round(sum((case      when (checkout like '2019-12-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as m12,   round(sum((case      when (checkout like '2019-%')      then datediff(checkout,checkin)*rate      else 0      end  )))  as Year    from lab7_reservations  inner join  lab7_rooms  on room =roomcode   group by room;";
